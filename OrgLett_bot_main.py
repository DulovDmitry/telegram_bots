...
ORGLETT_ASAP_URL = "https://pubs.acs.org/toc/orlef7/0/0"
CHANNEL_ID = -1002429466634
...
async def send_welcome(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, "You were unsubscribed.\nTo restart bot, send /start")
...
#	with open("/home/dima/tgbots/orglett_bot/test_res.txt") as textfile:
#		html_page = textfile.read()
...
#		await send_new_article_to_users()
	...
	async with async_playwright() as playwright:
		...
		await context.add_cookies(json.loads(Path("orglett_cookies.json").read_text()))
		page = await context.new_page()
		await page.goto(ORGLETT_ASAP_URL, timeout=0)
		time.sleep(10)
...
#			return
		...
		Path("orglett_cookies.json").write_text(json.dumps(cookies))
		await page.close()
...
async def fill_articles_table_in_database(articles_html):
	...
	for article in articles_html:
		if article.find('span', class_="issue-item_type").text == ", Articles ASAP  (Letter)":
...
#	user_id_list = database.get_all_user_ids()
...
async def main():
	...
	loop.create_task(sceduled_func(30)) # make request every 24 hours = 86400
